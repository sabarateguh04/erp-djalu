...
const emptyForm = { nama: "", perusahaan: "", email: "", telepon: "", alamat: "", sumber: "Referral", catatan: "" };

function CustomerModal({ initial, onClose, onSubmit, saving }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl w-full max-w-md p-5 shadow-xl">
        <h3 className="text-base font-semibold text-gray-800 mb-4">{initial ? "Edit Customer" : "Tambah Customer"}</h3>
...
export default function CustomersPage() {
  ...
  const handleSubmit = async (form) => {
    ...
    try {
      if (editing) {
        await crmApi.updateCustomer(editing.id, form);
      } else {
  ...
  const handleDelete = async (c) => {
    if (!confirm(`Hapus customer "${c.nama}"?`)) return;
    try {
      await crmApi.removeCustomer(c.id);
      await load();
    } catch (err) {
  ...
  const filtered = customers.filter((c) =>
    `${c.nama} ${c.perusahaan} ${c.email}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">CRM — Customer</h2>
          <p className="text-sm text-gray-400 mt-0.5">{customers.length} customer tercatat</p>
        </div>
        {canEdit && (
          <button
            onClick={() => {
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[11px] tracking-wider text-gray-400 border-b border-gray-100">
                  ...
                  <th className="pb-2 font-medium min-w-[140px]">PERUSAHAAN</th>
                  <th className="pb-2 font-medium min-w-[160px]">KONTAK</th>
                  <th className="pb-2 font-medium whitespace-nowrap">SUMBER</th>
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/60">
                    ...
                    <td className="py-3 pr-4 text-sm text-gray-600">{c.perusahaan || "-"}</td>
                    <td className="py-3 pr-4 text-sm text-gray-500">
                      {c.email || "-"} {c.telepon && `· ${c.telepon}`}
                    </td>
                    <td className="py-3 pr-4 text-sm text-gray-600 whitespace-nowrap">{c.sumber}</td>
                    {canEdit && (
